...
// CodeProject.AI is the actively maintained successor to the discontinued
// DeepStack and keeps its /v1/vision/face/* HTTP API, so this detector mirrors
// the DeepStack request/response handling. The default server port is 32168.
const { CODEPROJECTAI } = DETECTORS || {};

module.exports.recognize = async ({ key }) => {
  const { URL, KEY } = CODEPROJECTAI;
  const formData = new FormData();
  formData.append('image', fs.createReadStream(key));
  if (KEY) formData.append('api_key', KEY);
  return axios({
    method: 'post',
    timeout: CODEPROJECTAI.TIMEOUT * 1000,
    headers: {
      ...formData.getHeaders(),
    },
...
module.exports.normalize = ({ camera, data }) => {
  if (!data.success) {
    console.warn('unexpected codeprojectai data');
    return [];
  }
